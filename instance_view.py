...
with open(image_label_txt) as fp:
    ...
    for line in fp:
        ...
        for instance_class in np.unique(label):
            b = label.copy()
            image_copy = image.copy()

            current_class_seg = np.where(b == instance_class, b, 0)
            index = np.argwhere(current_class_seg == 0)

            for i, j in index:
                image_copy[i,j,:] = [0,0,0]

            label_colored = create_visual_anno(current_class_seg)
            align_image = np.hstack((image_copy, label_colored))
            align_image = np.hstack((image, align_image))
            # cv2.imwrite('', align_image)
